...
# Your Roku IP Address here
roku = Roku('your-roku-ip-address') # Find this in Roku's settings menu
...
# Pause/play Roku media
def pause_play():
    roku.play()
    return

# Open streaming service (currently not in use)
...
# Function call inside favorite_show()
def right_scroll():
    for _ in range(6):
        roku.right()
        time.sleep(0.1)
...
#     Run once background is captured
    if isBgCaptured == 1:
        ...
        # Prepare camera stream to input every 32nd frame into our VGG transformation pipeline
        frames.append(thresh)
        # copies 1 channel BW image to all 3 RGB channels
        input = np.stack((frames,) * 3, axis=-1)

        # Feed every 32nd frame into processing pipeline for our model
        if input.shape[0] == 32:
            ...
            if (result == 'Palm' and confidence >= .94):
                try:
                    action = "Volume down"
            ...
            elif (result == 'L' and confidence >= .98):
                try:
                    action = 'Volume up'
            ...
            # Designate Okay as 'No Gesture' because of excessively frequent false positive issues
                ...
                try:
                    action = 'Favorite Show'
                    favorite_show('I am not your negro')
